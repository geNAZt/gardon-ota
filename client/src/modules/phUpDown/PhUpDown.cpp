...
PhUpDown::PhUpDown(char upPin, char downPin, PhMeter* phMeter, CirculationPump* circulationPump) {
    this->_upPin = upPin;
...
void PhUpDown::loop(unsigned long millis) {
    if (PHUPDOWN_ENABLED == 0) {
        return;
    }
    ...
    if (this->_inProgressUntil == 0) {
        ...
        if (phValue > 0.1) {
            ...
            if (this->_downReachTarget > 0.1f) {
                debug("Pumpin for ph down ... target " + String(this->_downReachTarget, 2) + ", current " + String(phValue, 2));
                ...
                debug("Pumpin for ph up ... target " + String(this->_upReachTarget, 2) + ", current " + String(phValue, 2));
...
const char* PhUpDown::name() {
  return "PhUpDown";
}

String PhUpDown::metric() {
